...
const checkOwner = require('../src/auth/middlewares/checkOwner.js');
...
describe('checkOwner', () => {
  ...
  it('should success checkOuner', async () => {
    ...
    req.user = user;
    document.owner = {};
    return await checkOwner(req, res, next).then(() => {
  ...
  it('should get the room rout', async () => {
    ...
    const response = await mockRequest.get(`/`).set('params', `id ${room}`);
